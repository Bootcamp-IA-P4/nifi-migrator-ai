...
from app.rag.pdf_rag import build_pdf_rag_from_supabase # ✅ IMPORTAMOS EL RAG CORRECTO, SIN OPENAI
...
def get_combined_context(query: str, k: int = 5) -> str:
    ...
    try:
        ...
        local_context = f"⚠️ No se pudo obtener contexto local de FAISS: {e}"
        print(f"[ERROR] RAG local: {e}")

    try:
        qa_chain_from_pdfs = build_pdf_rag_from_supabase(bucket="flow")
        
        result = qa_chain_from_pdfs.invoke({"query": query})
        
        pdf_context_text = result.get('result', 'No se pudo generar contexto desde los PDFs.')

    except Exception as e:
        pdf_context_text = f"No se pudo generar contexto desde PDFs: {e}"
        print(f"[ERROR] RAG de PDFs: {e}")
